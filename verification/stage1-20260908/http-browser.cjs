...
const out=path.join(__dirname,label);fs.mkdirSync(out);const base=process.env.SHIYE_TEST_BASE||'http://127.0.0.1:4177';
const report={layer:'real local HTTP backend with synthetic fixtures; NOT real model validation',checks:[],errors:[],external:[],screenshots:[]};let browser;
...
async function flow(name,fn,options={}){
 ...
 await context.route('**/*',async route=>{
  ...
  if(url.pathname==='/api/segmentation/refine'&&request.method()==='POST')promptRequests.push(request.postDataJSON());
...
async function begin(p){await A(p,'home-create-sticker').click();await A(p,'seg-open').click();await A(p,'seg-start').click();await p.waitForFunction(()=>document.querySelectorAll('.seg-candidate').length===2&&[...document.querySelectorAll('.seg-source img,.seg-candidate img')].every(i=>i.complete&&i.naturalWidth>0));await p.waitForFunction(()=>!document.querySelector('[data-action="seg-confirm"]')?.disabled);}
async function main(){browser=await chromium.launch({channel:'chrome',headless:true});
 ...
 await flow('source-page-save-failure-retry',async p=>{
  ...
  await p.evaluate(()=>IDBObjectStore.prototype.put=window.testPut);await A(p,'seg-use').click();await p.locator('#canvas-page').waitFor();assert.ok((await p.locator('#canvas-page .element img').first().getAttribute('style')).includes('drop-shadow(0px 0 0 #fffdf6)'));const saved=await workspace(p),actual=saved.books.find(b=>b.id===book.id);assert.equal(actual.pages[0].id,book.pages[0].id);assert.equal(actual.pages[0].elements.length,2);assert.equal(saved.assets.length,2);
