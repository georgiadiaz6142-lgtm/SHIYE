...
files += [p for p in [runtime/'ai-copy-config.json',runtime/'ai-copy-prompt.json'] if p.exists()]
...
result={'pid':process.pid,'priorPid':pid,'adminUnchanged':unchanged[0],'invitationsUnchanged':unchanged[1],'environmentUnchanged':unchanged[2],'existingCopyConfigAndPromptUnchanged':all(unchanged[3:]),'serviceReady':True,'liveModePreserved':health.get('mode')=='live','callLimitSettingPreserved':health.get('limitsDisabled') is True}
(root/'verification/qwen-copy-20260910/restart-results.json').write_text(json.dumps(result,indent=2))
