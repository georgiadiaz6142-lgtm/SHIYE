...
(async()=>{
 ...
 await p.evaluate(()=>{const lake=samplePage('lake');currentBook().pages[1].elements=lake.elements;currentBook().pages[2].elements=clone(lake.elements);editing=false;renderEditor();cancelPageWarm();ShiyePageTextures.clear();});
 await p.evaluate(async()=>{await Promise.all([...document.images].map(i=>i.decode().catch(()=>{})));window.testTurn=prepareReaderTurn(1);testTurn.back.style.visibility='hidden';});
 await p.locator('.demo-fold-front').screenshot({path:path.join(__dirname,'before-reference.png')});
 ...
 for(let i=0;i<textures.length;i++)fs.writeFileSync(path.join(__dirname,`before-texture-${i}.png`),Buffer.from(textures[i].split(',')[1],'base64'));
 await p.evaluate(()=>drawSideTurn(testTurn,.8));await p.screenshot({path:path.join(__dirname,'before-turn.png')});
