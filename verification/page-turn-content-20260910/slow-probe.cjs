...
(async()=>{
 ...
 await context.route('**/assets/lake.jpg?slow',async route=>{await new Promise(r=>setTimeout(r,900));await route.fulfill({path:path.join(root,'shiye-editorial-prototype/assets/lake.jpg')});});
 ...
 for(let i=0;i<textures.length;i++)fs.writeFileSync(path.join(__dirname,`slow-texture-${i}.png`),Buffer.from(textures[i].split(',')[1],'base64'));
 await p.evaluate(()=>drawSideTurn(testTurn,.8));await p.screenshot({path:path.join(__dirname,'slow-turn.png')});
