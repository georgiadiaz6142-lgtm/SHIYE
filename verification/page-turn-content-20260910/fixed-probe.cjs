...
(async()=>{
 ...
 await context.route('**/assets/lake.jpg?slow',async route=>{await new Promise(r=>setTimeout(r,400));await route.fulfill({path:path.join(root,'shiye-editorial-prototype/assets/lake.jpg')});});
 await p.evaluate(()=>{const lake=samplePage('lake');currentBook().pages[1].elements=lake.elements;currentBook().pages[2].elements=clone(lake.elements);currentBook().pages[2].elements.find(e=>e.type==='photo').src='assets/lake.jpg?slow';editing=false;renderEditor();cancelPageWarm();ShiyePageTextures.clear();});
 await p.evaluate(async()=>{window.testTurn=prepareReaderTurn(1);testTurn.back.style.visibility='hidden';});

 await p.evaluate(()=>drawSideTurn(testTurn,0));await p.waitForFunction(()=>testTurn.ready);
 const textures=await p.evaluate(()=>[...testTurn.strips[0].querySelectorAll('.side-face')].map(i=>i.style.backgroundImage.match(/url\(["']?(.*?)["']?\)/)[1]));
 for(let i=0;i<textures.length;i++)fs.writeFileSync(path.join(__dirname,`fixed-texture-${i}.png`),Buffer.from(textures[i].split(',')[1],'base64'));
 await p.evaluate(()=>drawSideTurn(testTurn,.8));await p.screenshot({path:path.join(__dirname,'fixed-turn.png')});
 console.log(JSON.stringify({status:'captured',errors}));
