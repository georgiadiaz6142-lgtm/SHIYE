...
export function workRoutes(admin:Admin,works:Works){
 ...
 router.use((req,res,next)=>{const session=admin.session(adminToken(req.headers.cookie));if(!session)throw new Fault(401,'ACCOUNT_REQUIRED','请先登录账号后保存作品。');if(req.headers['x-shiye-work-account']&&req.headers['x-shiye-work-account']!==session.accountId)throw new Fault(409,'WORK_ACCOUNT_CHANGED','账号已切换，已停止原账号同步。');res.locals.workOwner=session.accountId;next();});
