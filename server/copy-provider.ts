...
export const QWEN_COPY_ENDPOINT='https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions';
export const QWEN_COPY_MODEL='qwen3.7-flash-2026-07-15';
export type CopyResult={text:string;provider:'ark'|'qwen'|'test';model:string;usage?:CopyUsage;requestId?:string};
...
class CompatibleCopyProvider implements CopyProvider {
 constructor(private key:string,private model:string,private request:typeof fetch=fetch,private prompt=DEFAULT_COPY_PROMPT,private providerId:'ark'|'qwen'='ark'){}
 async generate(input:CopyInput,signal:AbortSignal):Promise<CopyResult>{
  let response:Response;
  const qwen=this.providerId==='qwen';
  const format=qwen?{type:'json_schema',json_schema:{name:'shiye_copy',strict:true,schema:{type:'object',properties:{text:{type:'string'}},required:['text'],additionalProperties:false}}}:{type:'json_object'};
  try{response=await this.request(qwen?QWEN_COPY_ENDPOINT:ARK_COPY_ENDPOINT,{method:'POST',headers:{Authorization:'Bearer '+this.key,'Content-Type':'application/json'},body:JSON.stringify({model:this.model,messages:copyMessages(input,this.prompt),stream:false,max_tokens:512,response_format:format,...(qwen?{enable_thinking:false}:{})}),signal,redirect:'error'});}
  ...
  try{
   ...
   return {...result,provider:this.providerId,model:this.model,...(usage.success?{usage:usage.data}:{}),...(requestId?{requestId}:{})};
  }catch{throw new CopyProviderFault(502,'COPY_RESULT_INVALID','没有收到有效文案，本次不扣使用次数。',usage.success?usage.data:undefined,requestId);}
 }
}
export class ArkCopyProvider extends CompatibleCopyProvider {}
export class QwenCopyProvider extends CompatibleCopyProvider {
 constructor(key:string,model:string,request:typeof fetch=fetch,prompt=DEFAULT_COPY_PROMPT){super(key,model,request,prompt,'qwen');}
}
