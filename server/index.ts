...
const {host,port,runtime}=config, ttl=Number(process.env.SHIYE_TEMP_TTL_SECONDS||3600);
...
try {
  ...
  await admin.init({apiKey:process.env.BAIDU_API_KEY||'',secretKey:process.env.BAIDU_SECRET_KEY||'',cutout:mode==='live',naming:process.env.SHIYE_BAIDU_NAMING_ENABLED==='true'},config.administrator);
  if(live)live.provider=admin;
  const {app,jobs,copy}=await createApp({admin,access,runtime,publicOrigin:config.origin,staticRoot:resolve('shiye-editorial-prototype'),ttl:ttl*1000,mode,live,naming:mode==='live'?admin:undefined});
  const server=app.listen(port,host,()=>console.log(`拾页 ${config.deployment} 服务 ${config.origin||`http://${host}:${port}`}；${mode==='live'?'百度抠图，仅处理主动点击上传的照片。':'模拟模式，无外部模型调用。'}`));
