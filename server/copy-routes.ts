...
export function copyRoutes(admin:Admin,copy:AICopy){
 ...
 router.use(async(req,res,next)=>{
  const authorize=async():Promise<CopyIdentity>=>{
   const account=admin.session(adminToken(req.headers.cookie));let identity:CopyIdentity;
 ...
 router.post('/',express.json({limit:'256kb',strict:true,inflate:false}),async(req,res)=>res.status(202).json(await copy.submit(res.locals.copyAuthorize,req.body)));
