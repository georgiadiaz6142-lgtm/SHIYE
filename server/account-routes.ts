...
export function accountRoutes(admin:Admin,access:InviteAccess){
 ...
 router.get('/',async(req,res)=>{
  ...
  if(admin.session(token)){res.json({account:await admin.profile(token)});return;}
 ...
 router.post('/register',async(req,res)=>{
  if(admin.session(adminToken(req.headers.cookie)))throw new Fault(409,'ALREADY_REGISTERED','当前已经登录账号。');
  ...
  res.cookie('shiye_admin',token,{httpOnly:true,sameSite:'strict',path:'/api',maxAge:8*3600000});res.clearCookie('shiye_invite',{path:'/api'});res.json({account:await admin.profile(token)});
 });
 router.post('/logout',async(req,res)=>{
  const token=adminToken(req.headers.cookie);if(token)await admin.logout(token);
  res.clearCookie('shiye_admin',{path:'/api'});res.clearCookie('shiye_invite',{path:'/api'});res.json({ok:true});
 });
 router.use((req,res,next)=>{const token=adminToken(req.headers.cookie),session=admin.session(token);if(!session)throw new Fault(401,'ACCOUNT_REQUIRED','请先登录账号。');res.locals.accountToken=token;res.locals.accountId=session.accountId;next();});
 ...
 router.post('/password',async(req,res)=>{
  ...
  await admin.setPassword(res.locals.accountId,input.oldPassword,input.newPassword);res.clearCookie('shiye_admin',{path:'/api'});res.clearCookie('shiye_invite',{path:'/api'});res.json({ok:true});
 });
 return router;
}
