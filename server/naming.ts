...
export class Naming {
  ...
  async suggest(owner:string,sessionId:string,candidateId:string,revision:number,bytes:Buffer):Promise<string> {
    const s=this.jobs.session(owner,sessionId),candidate=s.candidates.find(c=>c.candidateId===candidateId&&c.candidateRevision===revision);
  ...
  private async run(owner:string,imageSessionId:string,key:string,expiresAt:number,image:Buffer) {
    ...
    const cached=await store.transaction(d=>{
      ...
      if([...this.pending.keys()].length>4)throw new Fault(429,'NAMING_BUSY','名称识别繁忙。');
    ...
    try{
      ...
      await store.transaction(d=>{const record=d.naming!.find(n=>n.key===key)!;Object.assign(record,{status:'succeeded',name,providerRequestId:result.requestId});});
      return name;
    }catch{
      await store.transaction(d=>{const record=d.naming!.find(n=>n.key===key)!;record.status='failed';});
      return '照片贴纸';
    }
  }
}
