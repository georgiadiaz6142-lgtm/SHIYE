...
export class Jobs {
  ...
  async upload(owner: string, operationId: string) {
    ...
    return this.store.transaction(async d=>{
      ...
      await writeFile(this.store.mediaPath(objectKey),await fixture(),{flag:'wx',mode:0o600});
  ...
  async submit(owner:string, input:StartInput|RefineInput, kind:'auto'|'refine') {
    ...
    const job=await this.store.transaction(d=>{
      ...
      s.latestJobId=job.jobId;d.jobs.push(job);return job;
    });
    if(job.status==='queued') this.enqueue(job.jobId);
    return job;
  }
  get(owner:string,id:string) {
    const j=this.store.data.jobs.find(j=>j.owner===owner&&j.jobId===id);
  ...
  async expire() {
    ...
    for(const id of obsolete)await unlink(this.store.mediaPath(id)).catch(e=>{if(e.code!=='ENOENT')throw e;});
  }
  async recover() {
    await this.expire();
  ...
  private async runBaidu(job:Job) {
    const session=this.session(job.owner,job.input.imageSessionId),input=job.input as RefineInput;
    const source=await readFile(this.store.mediaPath(session.objectKey));
    const dispatch=await this.store.transaction(d=>{
    ...
    await this.store.transaction(d=>{d.jobs.find(j=>j.jobId===job.jobId)!.providerRequestId=result.requestId;});
    const output=await compose(source,result.mask);
    await this.store.transaction(async d=>{
      const j=d.jobs.find(j=>j.jobId===job.jobId)!;
      if(j.status!=='running'||j.expiresAt<=Date.now())return;
      ...
      for(const [mediaId,bytes] of [[maskRef,result.mask],[transparentRef,output.png]] as const){
        await writeFile(this.store.mediaPath(mediaId),bytes,{flag:'wx',mode:0o600});d.media.push({mediaId,owner:s.owner,imageSessionId:s.imageSessionId,expiresAt:s.expiresAt});
      }
  ...
  private async run(id:string) {
    try {
      const job=await this.store.transaction(d=>{
        ...
        j.status='running';if(j.mock)j.providerRequestId=`mock:${j.jobId}`;j.updatedAt=Date.now();return j;
      });
      if(!job)return;
      if(job.provider==='baidu'){await this.runBaidu(job);return;}
      await delay(this.latency);
      const session=this.session(job.owner,job.input.imageSessionId);
      ...
      await this.store.transaction(async d=>{
        const j=d.jobs.find(j=>j.jobId===id)!;
        const s=this.session(job.owner,job.input.imageSessionId,d);
        if(j.status!=='running'||s.latestJobId!==id)return;
        for(const index of targets){
          ...
          for(const [mediaId,buffer] of [[maskRef,bytes],[transparentRef,output.png]] as const){
            await writeFile(this.store.mediaPath(mediaId),buffer,{flag:'wx',mode:0o600});d.media.push({mediaId,owner:s.owner,imageSessionId:s.imageSessionId,expiresAt:s.expiresAt});
          }
      ...
      await this.store.transaction(d=>{
        const j=d.jobs.find(j=>j.jobId===id);if(!j||!['queued','running'].includes(j.status))return;
