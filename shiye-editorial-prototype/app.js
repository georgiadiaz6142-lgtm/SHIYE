...
function renderWorkshop(){window.ShiyeEdge?.beforeRender();if(segmentedWorkshop?.active){segmentedWorkshop.render();return;}let w=workshop;let main='',aside='';
 ...
 if(w.step===0){main=`<div class="upload-zone" id="upload-zone"><div class="upload-symbol">${icon('image')}</div><h2>今天，想留住什么？</h2><p>放进一张照片，把喜欢的部分留下来。<br>支持 JPG、PNG、WebP，最大 10 MB。</p><button class="button dark" data-action="upload">${icon('upload')} 选择一张照片</button>${w.resumeStep?'<button class="sample-link" data-action="workshop-resume">继续上次制作</button>':''}<button class="sample-link" data-action="seg-open">进入自动抠图工坊</button></div>`;aside=`<div class="eyebrow">FROM PHOTO TO STICKER</div><h3 style="margin-top:13px">从照片里，<br>拾出一点喜欢。</h3><p>选出主体，调一圈白边，<br>就有了独一无二的收藏。</p><div class="note-rule"></div><div class="demo-label">正式流程：自动识别照片中的所有主要物体，再对漏提、错提对象圈选纠正，由 AI 贴合真实边缘。<br><b>百度真实测试的启用状态见自动抠图工坊。</b>个人照片可试用本地裁切和自动贴边试验，不代表 AI 效果。</div><p>${icon('lock')} 照片仅在当前浏览器处理。</p><button class="text-link" data-action="crop-example">用山湖照片试试圈选 ${icon('arrow')}</button>`;}
