...
document.addEventListener('click',async event=>{
 ...
 if(a==='nav'){event.preventDefault();if(btn.dataset.view==='admin'){await refreshIdentity();if(adminUser)location.assign('/admin');return;}await navigate(btn.dataset.view);return;}if(a==='about'){about();return;}if(a==='close-dialog'){pendingUseIds=[];closeDialog();return;}
...
document.addEventListener('change',e=>{
 if(!['new-text-font','selected-text-font','drawer-text-font'].includes(e.target.id))return;
 ...
 if(e.target.id==='new-text-font'){
  ...
  $('#'+e.target.id)?.focus({preventScroll:true});
 }
});
document.addEventListener('change',e=>{
 if(!['new-text-size','selected-text-size','drawer-text-size'].includes(e.target.id)||currentView!=='editor'||!editing)return;
 const size=e.target.valueAsNumber,item=currentElement(),isNew=e.target.id==='new-text-size';
 if(!Number.isFinite(size)||size<1||size>500){e.target.value=isNew?newTextSize:item?.size||23;toast('字号请输入 1～500');return;}
