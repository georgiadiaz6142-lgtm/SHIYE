...
function stickerCard(a,mode='library'){
 ...
 return `<article class="sticker-card" data-asset-id="${a.id}"><button class="sticker-display" data-action="${mode==='editor'?'insert-sticker':'sticker-detail'}" data-id="${a.id}" aria-label="${mode==='editor'?'添加':'查看'}${esc(a.name)}"><img src="${assetSource(a)}" alt="${esc(a.name)}" loading="lazy" draggable="false"><span class="add-badge">${icon(mode==='editor'?'plus':owned?'check':'plus')}</span></button><div class="sticker-label"><span>${ownedAsset?.pinned?'<small class="asset-pin" title="已置顶">'+icon('up')+'</small>':''}${esc(a.name)}</span><small>${esc(a.category)}</small></div>${mode==='library'&&owned?`<div class="sticker-card-actions"><button class="button outline small sticker-use" data-action="use-collected" data-id="${a.id}">用于创作</button><button class="icon-button sticker-more" popovertarget="sticker-menu-${a.id}" aria-label="${esc(a.name)}的更多操作" aria-expanded="false">⋯</button><div class="book-popover" id="sticker-menu-${a.id}" popover="auto" role="group" aria-label="${esc(a.name)}的操作"><button data-action="pin-sticker" data-id="${a.id}">${icon('up')} ${ownedAsset.pinned?'取消置顶':'置顶'}</button><button data-action="favorite-sticker" data-id="${a.id}">${icon('sticker')} ${ownedAsset.favorite?'取消收藏':'收藏'}</button><button class="book-menu-delete" data-action="delete-sticker-confirm" data-id="${a.id}">${icon('trash')} 删除</button></div></div>`:''}</article>`;
}
