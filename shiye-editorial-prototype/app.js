...
function filteredAssets(){
 ...
 return rows.map((a,index)=>({a,index})).filter(({a})=>(collectionTab!=='favorites'||a.favorite)&&(filter==='全部'||a.category===filter)&&(a.name.includes(search)||a.category.includes(search))).sort((x,y)=>collectionTab==='builtin'?x.index-y.index:Number(!!y.a.pinned)-Number(!!x.a.pinned)||(y.a.createdAt||y.index)-(x.a.createdAt||x.index)||y.index-x.index).map(({a})=>a);
}
function renderCollection(){shell(`<div class="page-heading"><div><h1>贴纸仓库<span style="color:var(--accent)">.</span></h1><p>生活里的小碎片，可以被一次次喜欢。</p></div><button class="button primary" data-action="nav" data-view="workshop">${icon('plus')} 制作贴纸</button></div><div class="collection-banner">${icon('sticker')}<p>把照片里的喜欢，变成自己的贴纸。<br>保存为素材后，可以放进任何一本手帐。</p><button class="button outline small" data-action="enter-creation">进入创作 ${icon('arrow')}</button></div><div class="collection-header"><div class="tabs"><button class="tab ${collectionTab==='favorites'?'active':''}" data-action="collection-tab" data-value="favorites">收藏 <span class="count">${state.assets.filter(a=>a.favorite).length}</span></button><button class="tab ${collectionTab==='mine'?'active':''}" data-action="collection-tab" data-value="mine">我的素材 <span class="count">${state.assets.length}</span></button><button class="tab ${collectionTab==='builtin'?'active':''}" data-action="collection-tab" data-value="builtin">拾页素材 <span class="count">${builtin.length}</span></button></div><label class="searchbox">${icon('search')}<input id="sticker-search" aria-label="搜索贴纸" placeholder="找一枚贴纸…" value="${esc(search)}"></label></div><div class="chips">${['全部','植物','物品','旅行','装饰','照片'].map(c=>`<button class="chip ${filter===c?'active':''}" data-action="filter" data-value="${c}">${c}</button>`).join('')}</div><div id="collection-results">${collectionResults()}</div>${footer()}`,'贴纸仓库');}
function collectionResults(){let a=filteredAssets();return a.length?`<div class="sticker-grid">${a.map(s=>stickerCard(s)).join('')}</div>`:`<div class="empty">${icon('sticker')}<h2>${search?'还没找到这枚贴纸':collectionTab==='favorites'?'还没有收藏的贴纸':'这里等着你的第一枚素材'}</h2><p>${collectionTab==='favorites'?'在素材的更多菜单中点击收藏，就会出现在这里。':'从拾页素材中挑选，或把自己的照片变成贴纸。'}</p><button class="button outline" data-action="nav" data-view="workshop">去制作贴纸 ${icon('arrow')}</button></div>`;}
...
async function inviteGranted(){try{const response=await fetch('/api/access/session',{credentials:'same-origin',cache:'no-store'});return response.ok&&(await response.json()).authorized===true;}catch{return false;}}
...
document.addEventListener('click',async event=>{
 ...
 if(a==='home-create-sticker'){await navigate('shelf');return;}
 if(a==='nav'){event.preventDefault();if(btn.dataset.view==='admin'){location.assign('/admin');return;}await navigate(btn.dataset.view);return;}if(a==='about'){about();return;}if(a==='close-dialog'){pendingUseIds=[];closeDialog();return;}
 ...
 if(a==='delete-book'){
  ...
  closeDialog();renderShelf();toast('手帐已删除，贴纸仓库仍保留');return;
 }
 ...
 if(a==='sticker-detail'){const s=asset(id),owned=state.assets.some(a=>a.id===id);showDialog(esc(s.name),`<div class="preview-stage" style="min-height:220px"><img src="${s.src}" alt="${esc(s.name)}" style="height:190px;width:70%;${borderStyle(s.borderBaked?0:s.border||0)}"></div><p class="dialog-description">${owned?'已保存在我的素材，可以反复放进不同手帐。':'拾页提供的原创示例插画。收藏后，可以反复放进不同手帐。'}</p>`,`<button class="button outline" data-action="close-dialog">关闭</button>${owned?`<button class="button outline" data-action="manage-sticker" data-id="${id}">整理</button>`:''}${owned?`<button class="button primary" data-action="use-collected" data-id="${id}">放进手帐 ${icon('arrow')}</button>`:`<button class="button primary" data-action="collect" data-id="${id}">${icon('plus')} 收入收藏</button>`}`);return;}
 if(a==='manage-sticker'){const s=asset(id);showDialog('整理这枚素材',`<label class="field"><span>贴纸名称</span><input id="sticker-rename" maxlength="30" value="${esc(s.name)}"></label><p class="dialog-description">删除素材不会影响已经放进手帐的贴纸。</p>`,`<button class="button outline" data-action="delete-sticker-confirm" data-id="${id}" style="margin-right:auto;color:var(--accent)">删除素材</button><button class="button primary" data-action="rename-sticker" data-id="${id}">保存名称</button>`);return;}
 if(a==='open-sticker-rename'){const item=state.assets.find(s=>s.id===id);if(!item)return;showDialog('重命名素材',`<label class="field"><span>贴纸名称</span><input id="sticker-rename" maxlength="30" value="${esc(item.name)}" autofocus></label>`,`<button class="button outline" data-action="close-dialog">取消</button><button class="button primary" data-action="rename-sticker" data-id="${id}">保存名称</button>`);$('#sticker-rename').select();return;}
 ...
 if(a==='pin-sticker'||a==='favorite-sticker'){
  const item=state.assets.find(s=>s.id===id);if(!item)return;btn.disabled=true;const key=a==='pin-sticker'?'pinned':'favorite';item[key]=!item[key];dirty();await saveNow();renderCollection();return;
 }
 if(a==='delete-sticker-confirm'){
  const item=state.assets.find(s=>s.id===id);if(!item)return;
  showDialog('删除这枚素材？',`<p class="dialog-description">将从我的素材和收藏中删除“${esc(item.name)}”。已放入手帐的贴纸仍会保留。</p>`,`<button class="button outline" data-action="close-dialog" autofocus>保留</button><button class="button danger" data-action="remove-sticker" data-id="${id}">删除</button>`);return;
 }
 if(a==='remove-sticker'){
  const item=state.assets.find(s=>s.id===id);if(item){state.archivedAssets=state.archivedAssets||[];const index=state.archivedAssets.findIndex(a=>a.id===id);if(index<0)state.archivedAssets.push({...item});else state.archivedAssets[index]={...item};state.assets=state.assets.filter(s=>s.id!==id);}
  closeDialog();dirty();const saved=await saveNow();renderCollection();if(saved)toast('已删除素材，手帐中的贴纸仍保留');return;
 }
 if(a==='collect'){const item=state.assets.find(s=>s.id===id);if(item)item.favorite=true;else state.assets.push({...asset(id),createdAt:Date.now(),favorite:true});closeDialog();dirty();await saveNow();renderCollection();toast('已收藏，可在收藏板块查看');return;}
 ...
 if(a==='save-stickers'||a==='save-and-use'){
  ...
  workshop.results.forEach(s=>{if(!state.assets.some(a=>a.id===s.id))state.assets.push({...s,border:workshop.border,createdAt:s.createdAt||Date.now()});});
  ...
  }else toast(`${ids.length} 枚贴纸已存入我的素材`);
  return;
 }
});
...
async function init(){
 ...
 try{const response=await fetch('/api/admin/session',{credentials:'same-origin',cache:'no-store',signal:AbortSignal.timeout(3000)});if(response.ok){const session=await response.json();adminUser=session.authenticated?session.username:null;}}catch{}
...
async function saveSegmentedAssets(results,use){
 ...
 try{
  for(const r of results){
   ...
   state.assets.push({id:r.id,name:r.name,category:r.category,createdAt:Date.now(),blobKey,border:r.border,borderBaked:true,width:r.width,height:r.height,provenance:r.provenance});
  }
  mutationVersion++;
  ...
  }else toast(`${ids.length} 枚${results.every(r=>r.provenance.mock)?'模拟':''}贴纸已存入我的素材`);
  return true;
 }finally{savingStickers=false;}
}
segmentedWorkshop=window.ShiyeSegmentation?.({esc,shell,toast,visible:()=>currentView==='workshop',origin:()=>workshopOrigin,restoreOrigin:origin=>{if(!workshopOrigin)workshopOrigin=origin;},renderLegacy:renderWorkshop,save:saveSegmentedAssets,completed:id=>[...(savedWorkspace?.assets||[]),...(savedWorkspace?.archivedAssets||[])].some(a=>a.provenance?.imageSessionId===id)});
init();
